...
export async function GET(req: NextRequest) {
  try {
    ...
    const circuitsAvecStats = circuitsAvecReservationsPop.map(circuit => ({
      id: circuit.id,
      title: circuit.title,
      nombreReservations: circuit.reservations.length,
      nombreParticipants: circuit.reservations.reduce((sum, res) => sum + res.personnes, 0)
    })).sort((a, b) => (b.nombreReservations + b.nombreParticipants) - (a.nombreReservations + a.nombreParticipants))
    .slice(0, 5)
    ...
    const tripsAvecStats = tripsAvecReservationsPop.map(trip => ({
      id: trip.id,
      title: trip.title,
      nombreReservations: trip.reservations.length,
      nombreParticipants: trip.reservations.reduce((sum, res) => sum + res.personnes, 0)
    })).sort((a, b) => (b.nombreReservations + b.nombreParticipants) - (a.nombreReservations + a.nombreParticipants))
    .slice(0, 5)
    ...
    reservationsAvecPrix.forEach(reservation => {
      ...
      if (reservation.circuitRel) {
        prix = parseFloat(reservation.circuitRel.price) || 0
      } else if (reservation.TripTravel && reservation.travelDate?.price) {
        prix = reservation.travelDate.price || 0
      } else if (reservation.TripTravel) {
        prix = parseFloat(reservation.TripTravel.price) || 0
      }
      
      totalRevenus += prix * reservation.personnes
    })
    ...
    const moisUniques = [...new Set(moisDisponibles.map(m => 
      m.createdAt.toISOString().substring(0, 7)
    ))].sort().reverse()

    const anneesDisponibles = [...new Set(moisDisponibles.map(m => 
      m.createdAt.getFullYear().toString()
    ))].sort().reverse()
